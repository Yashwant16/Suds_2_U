...
const UploadDriverLicense = ({ navigation, route }) => {
  ...
  const onSubmit = async data => {
    ...
    setLoading(true);
    let success = await updateDrivingLicense({ ...data,issued_on : moment(issuedOn).format('YYYY-MM-DD'),expiry_date : moment(expirationDate).format('YYYY-MM-DD'),   image: selectedImage });
    // if (success) await documentVerified()
    if (success) route.params?.authStack ? navigation.goBack() : navigation.goBack();
    setLoading(false);
  };
  ...
  const [expirationDatePickerVisible, setExpirationDatePickerVisibility] = useState(false)

  return (
    <SafeAreaView style={{ flex: 1 }}>
      <ScrollView contentContainerStyle={{flexGrow : 1,backgroundColor :'white'}} keyboardShouldPersistTaps="handled">
        {issuedOnPickerVisible && <DateModal value={issuedOn} confirm={setIssuedOn} dismiss={() => setIssuedOnPickerVisibility(false)} />}
        {expirationDatePickerVisible && <DateModal value={expirationDate} confirm={setExpirationDate} dismiss={() => setExpirationDatePickerVisibility(false)} />}
        <LoadingView loading={loading} fetchingColor={Colors.blue_color} fetching={fetching}>

          <View style={{ height: 250, width: '100%', backgroundColor: '#eee', justifyContent: 'center', alignItems: 'center' }}>

            {
              selectedImage ? (
                <View style={{ flex: 1, height: 250, width: '100%', alignItems: 'center', justifyContent: 'center' }}>
                  <Image
                  ...
                  />
                  <TouchableOpacity activeOpacity={0.9} onPress={() => launchImageLibrary({}, imageSelectCallBack)} style={{ borderRadius: 100, height: 150, width: 150, backgroundColor: '#00000050', alignItems: 'center', justifyContent: 'center', position: 'absolute', opacity: .8 }}>
                    <Icon color="white" size={50} name="autorenew" />
                    <Text style={{ color: 'white', textAlign: 'center', fontSize: 14 }}>Change the photo</Text>
                  </TouchableOpacity>
                </View>

              ) : (
                <TouchableOpacity activeOpacity={0.9} onPress={() => launchImageLibrary({}, imageSelectCallBack)} style={{ borderRadius: 100, height: 150, width: 150, backgroundColor: 'white', alignItems: 'center', justifyContent: 'center', elevation: 10, shadowColor: '#555' }}>
                  <Icon color="#aaa" size={50} name="add" />
                  <Text style={{ color: '#aaa', textAlign: 'center', fontSize: 14 }}>Add a photo</Text>
                </TouchableOpacity>
              )
            }
          ...
          <View style={{ flex: 1, backgroundColor: '#FFF', alignItems: 'center', paddingTop: 16 }}>
            ...
            <View style={{ width: '90%', paddingHorizontal: 2 }}>
              <Text style={{ color: '#999', textAlign: 'left', width: '100%', marginTop: 4 }} >ISSUED ON</Text>
              <TouchableOpacity onPress={() => setIssuedOnPickerVisibility(true)} style={{ padding: 8, paddingHorizontal: 0, borderBottomWidth: 1, borderBottomColor: '#ddd' }}>
                <Text style={{ fontSize: 16, color: issuedOn ? 'black' : '#999' }}>{issuedOn ? issuedOn.toDateString() : '- - - - / - - / - -'}</Text>
              </TouchableOpacity>
            </View>
            <View style={{ width: '90%', paddingHorizontal: 2 }}>
              <Text style={{ color: '#999', textAlign: 'left', width: '100%', marginTop: 4 }} >EXPIRATION DATE</Text>
              <TouchableOpacity onPress={() => setExpirationDatePickerVisibility(true)} style={{ padding: 8, paddingHorizontal: 0, borderBottomWidth: 1, borderBottomColor: '#ddd' }}>
                <Text style={{ fontSize: 16, color: expirationDate ? 'black' : '#999' }}>{expirationDate ? expirationDate.toDateString() : '- - - - / - - / - -'}</Text>
              </TouchableOpacity>
            </View>
            {/* <ControllerInput
              label="ISSUED ON"
          ...
          <View style={{ justifyContent: 'center', alignItems: 'center', backgroundColor: 'white', borderRadius: 15, padding: 10, position: 'absolute' }}>
            ...
            <View style={{ flexDirection: 'row' }}>
              <TouchableOpacity onPress={dismiss} style={{ padding: 10, borderRadius: 5, flex: 1, borderWidth: 2, borderColor: Colors.privacypolicy_headingcolor + '50' }}>
                <Text style={{ fontFamily: 'Montserrat-SemiBold', textAlign: 'center' }}>Cancel</Text>
              </TouchableOpacity>
              <View style={{ width: 10 }} />
              <TouchableOpacity onPress={onConfirm} style={{ backgroundColor: Colors.blue_color, padding: 10, borderRadius: 5, flex: 1 }}>
                <Text style={{ fontFamily: 'Montserrat-SemiBold', textAlign: 'center', color: 'white' }}>Confirm</Text>
              </TouchableOpacity>
            </View>
          </View>
        </TouchableOpacity>
